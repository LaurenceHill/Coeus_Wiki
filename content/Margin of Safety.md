...
**Source:** [[Berkshire Chairman's Letters]] — 1990 letter, junk bond
section (Graham quotation); 1992 letter, "Common Stock Investments"
...
it been applied, would have prohibited the construction of those
...
unavoidable — do not convert an apparently attractive investment into
a poor one. The margin is the buffer between the purchase price and
the estimated value; its purpose is to absorb estimation error.
...
estimated fair value is not — any estimation error converts the
investment into an overpayment.

Graham's original context was primarily about securities with
identifiable hard assets — bonds, preferred stocks, statistically
cheap common stocks. Buffett's application is broader: even for
businesses evaluated on a discounted-cash-flow basis, requiring a
meaningful gap between the purchase price and the calculated value
is the correct discipline. The uncertainty in the DCF estimate makes
the gap more important, not less.

## What constitutes an adequate margin

Buffett does not specify a fixed percentage, and the appropriate
margin varies by the reliability of the underlying estimate. The
...
Fisher's [[Financial Community Appraisal]] framework adds a
complementary dimension: the investor who can correctly identify
businesses where the financial community's current appraisal is
materially wrong will find that the margin of safety is already
partly embedded in the price — the market is doing part of the
work by underestimating the business. The margin-of-safety
requirement then functions as a further screen against situations
where the investor's own assessment is correct but the gap is
insufficient.

## The junk bond context: margin of safety as a structural requirement
...
intrinsic value to absorb valuation errors. The firm requires a gap
between its cash generation and its debt service obligations to absorb
operating variability. In both cases, the margin is the difference
between the scenario required for survival and the scenario most likely
to occur. Where that difference is small, a small adverse development
produces failure; where the margin is wide, the same development is
...
tolerable, which means that the investor should either raise the certainty
threshold or reduce activity. Both responses point in the same direction:
wait for the fat pitch.

## Relationship to the growth-vs-value framework

The 1992 letter embeds the margin-of-safety principle directly in the
growth-vs-value argument. Having established that value and growth are
a unified calculation based on John Burr Williams's DCF formula,
Buffett adds: the correct investment is the one whose discounted cash
flows most exceed the purchase price, regardless of whether it is
labelled "growth" or "value." The margin of safety is the operational
expression of that argument: pay less than the calculated value, and
give yourself room for error. See [[Growth vs Value False Dichotomy]].

## Related

- [[Berkshire Chairman's Letters]] — primary source (1990 Graham
  quotation; 1992 explicit naming)
- [[Intrinsic Value vs Book Value]] — the intrinsic value calculation
  is the reference point from which the margin is measured
- [[Owner Earnings]] — the more precise earnings metric that feeds the
  intrinsic value estimate; a better estimate requires less margin
- [[Growth vs Value False Dichotomy]] — the DCF unifying framework
  that the margin-of-safety principle disciplines
- [[Financial Community Appraisal]] — the mechanism by which markets
  sometimes provide a margin of safety for free, via mispricing
- [[Leverage and Liquidity]] — the financial-architecture counterpart:
  surplus liquidity is the balance-sheet version of margin of safety;
  both protect against the unforeseeable
- [[Mr. Market]] — the psychological mechanism underlying why prices
  periodically diverge from value far enough to create large margins
- [[Franchise vs Business Economics]] — the franchise category is the
  one where the intrinsic value estimate is most reliable, and therefore
  where a smaller margin of safety may be appropriate
- [[Denial, Anchoring, Drift]] — the failure mode on the opposite side:
  holding a position whose margin of safety has been consumed without
  acknowledging the deterioration
- [[Concentration vs Diversification]] — concentration is appropriate
  only when the analytical work is deep enough to support confidence in
  both the intrinsic value estimate and the margin
- [[Circle of Competence]] — the prerequisite: restricting the universe
  to understandable businesses reduces the magnitude of margin required
- [[Patience as Competitive Advantage]] — the fat-pitch discipline and
  Sleep's low-turnover edge are the same idea from different angles
